...
def run_many_times(file_path):
    ...
    result_record = pd.DataFrame([], columns=['seed', 'best_accr', 'best_f1', \
        ...
        'dummy_accr_sw', 'dummy_f1_sw', 'best_accr_mw', 'best_f1_mw',\
    ...
    for i in range(run_time):
        ...
        result_record.to_csv('./output/result_baseline_ood_{0}.csv'.format(start_seed))
...
def load_data(file_path):
    # base_df = pd.read_csv('data/valid_data_baseline.csv')
    df = pd.read_csv(file_path)
...
def main():
    file_path = 'data/valid_data_baseline.csv'
