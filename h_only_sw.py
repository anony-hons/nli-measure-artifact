...
def run(input_sen, output, testloaders, logit_df, seed=42):
    ...
    return {
        ...
        'best_accr_mw': test_measures['ood_mw']['accr'],
        'best_f1_mw': test_measures['ood_mw']['f1'],
        'dummy_accr_mw': test_measures['ood_mw']['dummy_accr'],
        'dummy_f1_mw': test_measures['ood_mw']['dummy_f1'],
        # 'majority': y_majority
    }
...
def run_many_times(file_path):
    ...
    result_record = pd.DataFrame([], columns=['seed', 'best_accr', 'best_f1', \
        'dummy_accr', 'dummy_f1', 'best_accr_bs', 'best_f1_bs',\
        'dummy_accr_bs', 'dummy_f1_bs', 'best_accr_mw', 'best_f1_mw',\
        # 'best_accr_mmatch', 'best_f1_mmatch', 'best_accr_mismatch', 'best_f1_mismatch',\
        'dummy_accr_mw', 'dummy_f1_mw'])
    testloaders = {
        'ood_bs': load_test_data('./data/valid_data_baseline.csv'),
        'ood_mw': load_test_data('./data/valid_data_multiart.csv')
    }
    ...
    for i in range(run_time):
        ...
        result_record.to_csv('./output/result_cond1_ood_{0}.csv'.format(start_seed))
...
def load_data(file_path):
    # base_df = pd.read_csv('data/valid_data_cond1.csv')
    df = pd.read_csv(file_path)
...
def main():
    file_path = 'data/valid_data_cond1.csv'
